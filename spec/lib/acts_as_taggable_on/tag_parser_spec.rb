...
RSpec.describe ActsAsTaggableOn::TagParser do
  let_it_be(:tag0) { create(:tag, name: "things") }
  let_it_be(:tag1) { create(:tag, name: "peter") }

  let_it_be(:tag2) { create(:tag, name: "stuff", alias_for: tag0.name) }

  let_it_be(:tag3) { create(:tag, name: "mac", alias_for: tag1.name) }
